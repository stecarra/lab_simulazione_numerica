...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double prob_plus = 1 / (1 + exp(-2 * _beta * _J * (_particle[this->pbc(i-1)].getspin() 
                                    + _particle[this->pbc(i+1)].getspin()) - 2*_beta*_H));
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8*M_1_PI*_rho/3 * (pow(_r_cut, -9)/3 - pow(_r_cut, -3)); // TO BE FIXED IN EXERCISE 7
        ...
        index_property++;
        _ptail = 32*M_1_PI*_rho/3 * (pow(_r_cut,-9)/3 - pow(_r_cut,-3)/2); // TO BE FIXED IN EXERCISE 7
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG/config.xyz");  //config.ising o config.xyz
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
...
void System :: measure(){ // Measure properties
  ...
  vec gofr(_n_bins, arma::fill::zeros);

  //ofstream out;
  //out.open("dr"); output di debugging per contrrollare come sono distribuite le distanze
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(dr < _halfside(0)){
          temp_index = static_cast<int>(_n_bins * dr / _halfside(0));
          gofr(temp_index) += 2; // filling gofr histogram
        }

        if(dr < _r_cut){
          if(_measure_penergy)  penergy_temp += 1.0/pow(dr,12) - 1.0/pow(dr,6); // POTENTIAL ENERGY
          if(_measure_pressure){
            pressure_temp += 48 * (1.0/pow(dr,12) - 0.5/pow(dr,6)); // PRESSURE , using sigma e epsilon = 1
            //out<<dr<<endl;
          }
    ...
    //scaling gofr histogram
    double denominator;
    for(int i=0; i<_n_bins; i++){
      denominator = (_rho * _npart) * 4 * M_1_PI / 3 *(pow(double(i+1)/_n_bins*_halfside(0),3)-pow(double(i)/_n_bins*_halfside(0),3));
      gofr(i) = gofr(i) / denominator;
    }
    for(int i=0; i<_n_bins; i++){
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 4, using kb=1
  if (_measure_pressure and _measure_temp ) _measurement(_index_pressure) = _rho * _measurement(_index_temp) + pressure_temp /(3* _volume);
  // MAGNETIZATION /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  if (_measure_magnet){
    for(int i=0; i<_npart; i++){
...
// TO BE FIXED IN EXERCISE 6
  if (_measure_chi){
    ...
    _measurement(_index_chi) = _beta * pow(magnetization,2); 
    magnetization = 0.0;
  }
...
void System :: averages(int blk){
  ...
  //correggo il vettore _average tenendo conto del fatto che in _block_av npn c'è cv ma H^2 
  if(_measure_cv){
  ...
  // TO BE FIXED IN EXERCISE 7
    ...
    for(int i=0; i<_n_bins; i++){
      ...
      coutf << setw(12) << (i + 0.5) / _n_bins * _halfside(0)
            << setw(12) << sum_average/double(blk)
            << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;     
    }
    coutf.close();
